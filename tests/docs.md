...
- Exercises the compiled binary produced from `@/src/main.rs` -- tests the full end-to-end pipeline including file discovery, rule execution, output formatting (text and JSON), directory argument handling, config handling, and exit codes
...
- Helper functions create SKILL.md fixtures that target specific rule behaviors: valid skills (small, well-formed), oversized skills (exceeding line limit), skills missing `<required>` tags, and skills with unclosed tags
- Tests are organized into groups covering: default/text format output, directory argument handling, `--format json` output, per-rule violation detection, and `--config` flag behavior
- Config integration tests verify: deterministic rules still run without config, `--config` with nonexistent file errors, invalid JSON errors, missing `anthropic_api_key` field errors, `--config=` equals syntax, `--config` with missing value, and auto-discovery of `config.json` in the working directory
...
- The auto-discovery test (`config_json_in_cwd_is_auto_discovered`) writes a deliberately invalid config to prove the file was loaded: the test expects a failure with an `anthropic_api_key` error message
