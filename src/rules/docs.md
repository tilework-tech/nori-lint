...
- **Deterministic rules** -- Exported as constant objects conforming to `Rule`. Each receives full file content as a string and returns an array of violations. Rules that implement `fix` can auto-correct violations: `bold_italics` (strips bold/italic markdown formatting), `markdown_links` (replaces `[text](url)` with bare URLs), `redundant_title` (removes redundant title headings), `consecutive_blank_lines` (collapses 3+ consecutive newlines to 2 via regex replacement), `trailing_whitespace` (strips trailing spaces/tabs per line), and `when_to_use` (removes "When to Use" sections since this information belongs in the frontmatter `description` field). Rules without `fix` (`description_action`, `frontmatter`, `frontmatter_name_format`, `line_count`, `required_tags`, `unclosed_tags`) are reported as unfixable during `nori-lint fix`.
...
- The `consecutive_blank_lines` and `trailing_whitespace` rules are token-efficiency rules. Unlike content-oriented rules, they operate on raw whitespace characters without parsing markdown structure. `consecutive_blank_lines.fix()` uses a single regex (`/\n{3,}/g`) to collapse runs; `trailing_whitespace.fix()` maps over individual lines to strip trailing `[ \t]+$`.  The `trailing_whitespace` rule explicitly skips empty lines (length 0) to avoid false positives on blank-line separators.
...
- The `when_to_use` rule is the complement to `description_action`: `description_action` enforces that "when to use" information is present in the frontmatter `description` field, while `when_to_use` enforces that a separate "When to Use" section does not exist in the body (since it would be redundant). The `when_to_use` fix removes the entire section from the matched heading through to the next heading at the same or higher level (or EOF), skipping headings inside code blocks. After removal, triple-or-more blank lines are collapsed to double.
