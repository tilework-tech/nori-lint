...
- System prompts include explicit carve-outs for content that should NOT be flagged (e.g., project-specific terms in `redundant_explanation`, absolute prohibitions in `negative_without_positive`, single contextual CLI examples for `cli_command_index`) to reduce false positives.
