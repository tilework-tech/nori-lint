...
mod tests {
    ...
    fn returns_violation_for_atx_heading_at_start() {
        ...
        assert!(violation.line.is_some(), "should report a line number");
        assert_eq!(violation.line.unwrap(), 1);
        assert!(
            violation.snippet.is_some(),
            "should include the heading as snippet"
        );
        assert!(violation.snippet.unwrap().contains("# The Foo Document"));
    }

    #[test]
    fn returns_none_for_body_text_at_start() {
        let rule = RedundantTitleRule;
    ...
    fn returns_violation_for_heading_after_frontmatter() {
        ...
        let result = rule.run(content);
        assert!(result.is_some(), "should flag heading after frontmatter");
        let violation = result.unwrap();
        assert_eq!(violation.line.unwrap(), 6, "heading is on line 6");
        assert!(violation.snippet.unwrap().contains("# The Foo Document"));
    }

    #[test]
    fn returns_none_for_body_text_after_frontmatter() {
        let rule = RedundantTitleRule;
        let content =
            "---\nname: Foo\ndescription: A foo skill\n---\n\nThis document describes foo.\n";
        assert!(rule.run(content).is_none());
    ...
    fn returns_violation_for_h2_heading() {
        ...
        let result = rule.run(content);
        assert!(result.is_some(), "should flag any heading level");
        let violation = result.unwrap();
        assert_eq!(violation.line.unwrap(), 1);
        assert!(violation.snippet.unwrap().contains("## Section Title"));
    }
    ...
    fn returns_violation_for_setext_h1() {
        ...
        let content = "The Foo Document\n================\n\nThis describes foo.\n";
        let result = rule.run(content);
        assert!(result.is_some(), "should flag setext h1 heading");
        let violation = result.unwrap();
        assert_eq!(violation.line.unwrap(), 1);
        assert!(violation.snippet.unwrap().contains("The Foo Document"));
    }
    ...
    fn returns_violation_for_setext_h2() {
        ...
        let content = "The Foo Document\n----------------\n\nThis describes foo.\n";
        let result = rule.run(content);
        assert!(result.is_some(), "should flag setext h2 heading");
        let violation = result.unwrap();
        assert_eq!(violation.line.unwrap(), 1);
        assert!(violation.snippet.unwrap().contains("The Foo Document"));
    }
    ...
    fn returns_violation_for_setext_heading_after_frontmatter() {
        ...
        let violation = result.unwrap();
        assert_eq!(violation.line.unwrap(), 6);
        assert!(violation.snippet.unwrap().contains("The Foo Document"));
    }
    ...
    fn does_not_confuse_frontmatter_delimiter_with_setext() {
        ...
        let content = "---\nname: Foo\n---\n\nThis is body text.\n";
        assert!(
            rule.run(content).is_none(),
            "frontmatter delimiter should not be treated as setext underline"
        );
    }

    #[test]
    fn returns_none_for_no_content_after_frontmatter() {
        let rule = RedundantTitleRule;
        let content = "---\nname: Foo\n---\n";
        assert!(rule.run(content).is_none());
    }
    ...
    #[test]
    fn returns_none_when_heading_appears_later_in_body() {
        let rule = RedundantTitleRule;
        let content = "---\nname: Foo\ndescription: A foo skill\n---\n\nThis is body text.\n\n# Section Heading\n\nMore content.\n";
        assert!(
            rule.run(content).is_none(),
