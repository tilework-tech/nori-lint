...
- **`cli.ts`** -- Async entry point. Uses `commander` with a subcommand pattern: the root program shows help when invoked with no arguments. Two subcommands are registered: `lint` (report violations) and `fix` (auto-fix violations). Shared setup logic (config resolution, registry construction, file discovery, rule validation) is extracted into `setupCommand()` to avoid duplication between subcommands. The `lint` subcommand supports `--format text|json` and `--config`; the `fix` subcommand supports `--dry-run` and `--config`. During `fix`, deterministic rules with a `fix` method are applied in-process; LLM violations are batched and fixed via a single `llmClient.fixContent()` call per file, with code block masking applied around that call (see `code-block-mask.ts`). Rules without `fix` are reported as unfixable to stderr.
- **`code-block-mask.ts`** -- Provides `maskCodeBlocks()` and `restoreCodeBlocks()` to protect fenced code blocks from LLM modification during the fix phase. `maskCodeBlocks` line-scans for `` ``` `` fences, extracts complete blocks (including fences), stores them in an array, and replaces them with HTML comment placeholders (`<!-- NORI_CODE_BLOCK_N -->`). Unclosed code blocks at EOF are left in place. `restoreCodeBlocks` replaces each placeholder with the original block content, throwing an error if any placeholder was removed by the LLM. Used exclusively from `cli.ts` at the `fixContent()` call site, keeping `llm-client.ts` unaware of masking.
