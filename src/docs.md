...
- **`cli.ts`** -- Async entry point. Uses `commander` with a subcommand pattern: the root program shows help when invoked with no arguments, while `program.command("lint")` registers the lint subcommand carrying all options (`--format`, `--config`) and the `[path]` argument. Uses `parseAsync` to support the async action callback. Orchestrates: config resolution, registry construction, file discovery via `globSync("**/SKILL.md")`, deterministic rule execution (synchronous per file), LLM rule execution (concurrent via `Promise.all`), and output formatting (text or JSON to stdout).
